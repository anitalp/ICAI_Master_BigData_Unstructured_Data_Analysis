...
htemp <- list()
htags <- data.frame()
data <- read.csv("table-juanllorca.csv", sep = ",") #,
...
write.csv(htags, "ht_unsort_juanllorca.csv", fileEncoding = "UTF-8")
df_htags <- as.data.frame(table(unlist(htags)))
write.csv(df_htags, "ht_sort_juanllorca.csv", fileEncoding = "UTF-8")